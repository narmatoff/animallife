...
<xsl:template match="item" mode="comments.list">
                <div class="comment">
                    <h3><xsl:apply-templates select="document(concat('udata://users/viewAuthor/', @author_id))/udata"/><a name="{@id}" ></a>
                    <!-- Вывод красивой русской даты. Как лучше??? -->
                      <span><xsl:value-of select="document(concat('udata://system/convertDate/', @publish_time, './(j)'))/udata"/>
                      <xsl:choose>
                      ...
                      <xsl:value-of select="document(concat('udata://system/convertDate/', @publish_time, './(Y)'))/udata"/></span>
                    </h3>
                    <div class="comment-text">

                      <div class="comment-bar col-{@col}"></div>
...
<xsl:template match="user[@type = 'guest']" mode="addcomment">
                            ...
                            <!-- <form class="std-block firm-send" method="post" action="{.}" name="form_addcomment" id="form_addcomment"> -->
                                <div class="std-row">
    ...
    <xsl:apply-templates select="document('udata://system/captcha')/udata[url]" />
                                <div class="std-row radio-wrp">
                                    <label class="col-n"><input type="radio" name="col" id="col" class="style-me" value="n"/>нейтральный</label>
                                    <label class="col-g"><input type="radio" checked = "checked" name="col" id="col" class="style-me" value="g"/>положительный</label>
                                    <label class="col-b"><input type="radio" name="col" id="col" class="style-me" value="b"/>отрицательный</label>
                                </div>
                                <div class="std-row">
                                  <button class="std-btn" type="submit">Опубликовать</button>
                                </div>
                            </form>
</xsl:template>
